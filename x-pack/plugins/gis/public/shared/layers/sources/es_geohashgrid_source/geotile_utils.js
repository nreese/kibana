...
export function getTilePolygon(tileKey) {
  const { x, y, zoom, tileCount } = parseTileKey(tileKey);

  const top = tileToLatitude(y, tileCount);
  const bottom = tileToLatitude(y + 1, tileCount);
  const left = tileToLongitude(x, tileCount);
  const right = tileToLongitude(x + 1, tileCount);
...
//const centerLat = tileToLatitude(y+0.5, tileCount);
//const centerLng = tileToLongitude(x+0.5, tileCount);
